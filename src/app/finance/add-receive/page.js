...
const ClientForm = ({ selectedLoja, onSuccessRedirect, userId, userName }) => {
    ...
    // Variáveis para busca de cliente - exatamente como no add-receive
    const [searchTerm, setSearchTerm] = useState('');
    const [consumers, setConsumers] = useState([]);
    const [isSearching, setIsSearching] = useState(false);
    const [selectedTitular, setSelectedTitular] = useState('');

    // Função para buscar clientes - COPIADA EXATAMENTE do add-receive
    ...
    // Efeito para buscar clientes - IDENTICO ao add-receive
    useEffect(() => {
        fetchConsumers();
    ...
    // Função para selecionar um cliente dos resultados da busca - IDÊNTICA ao add-receive
        ...
        <form onSubmit={handleSubmit} className="mt-8 mb-20">
                        ...
                        <div className="space-y-4">
                            <div className="space-y-2 relative mb-20">
                                ...
                                <div className="relative">
                                        ...
                                        <div className="absolute w-full z-10">
                                            <ul className="bg-white border-2 border-[#81059e] rounded-sm w-full max-h-[104px] overflow-y-auto shadow-lg custom-scroll">
                                                {consumers.map((consumer) => (
                                                    <li
                                                        key={consumer.id}
                                                        onClick={() => handleSelectCliente(consumer)}
                                    ...
                                    {searchTerm && consumers.length === 0 && !isSearching && (
                                        <div className="absolute w-full z-10">
            ...
            {showSuccessPopup && (
                <div className="fixed top-4 right-4 z-50 animate-fade-in">
                    <div className="bg-green-500 text-white px-6 py-4 rounded-sm shadow-lg flex items-center gap-2">
