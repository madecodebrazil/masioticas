  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const sangriaData = {
        ...
        valorFinal: -Math.abs(parseFloat(amount)), // Valor negativo para a sangria no caixa
        lojas: store,
      };

      console.log("Dados da Sangria:", sangriaData);

      // Envia os dados para a coleção 'sangrias'
      const sangriaRef = await addDoc(collection(firestore, 'sangrias'), sangriaData);
      const sangriaId = sangriaRef.id;

      // Dados para o cashflow (marcado como isSangria)
      const cashflowData = {
        sangriaId: sangriaId,
        data: saleTimestamp,
        descricao: `Sangria para ${description}`,
        formaPagamento: 'Sangria',
        valorFinal: -Math.abs(parseFloat(amount)), // Valor positivo para exibição no cashflow
        tipo: 'Sangria',
        lojas: store,
        isSangria: true, // Identificador para saber que é uma sangria
      };
      ...
      const caixaTransactionData = {
        sangriaId: sangriaId,
        data: saleTimestamp,
        valor: -Math.abs(parseFloat(amount)), // Valor negativo para o caixa
        descricao: `Sangria para ${description}`,
        tipo: 'Sangria',
      };
      ...
      alert('Erro ao registrar a sangria. Tente novamente.');
    }
  };
