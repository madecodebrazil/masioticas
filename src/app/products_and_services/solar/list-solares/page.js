...
export default function ListaSolares() {
  const router = useRouter();
  const { userPermissions, userData } = useAuth();
  const [solares, setSolares] = useState([]);
  const [filteredSolares, setFilteredSolares] = useState([]);
  ...
  const [totalSolares, setTotalSolares] = useState(0);
  ...
  const [editingSolar, setEditingSolar] = useState(null);
  ...
  useEffect(() => {
    const fetchSolares = async () => {
      try {
        setLoading(true);
        const fetchedSolares = [];

        // Usar o caminho correto para lojas específicas
        if (selectedLoja !== 'Ambas') {
          // Caminho para uma loja específica
          const solaresDocRef = collection(firestore, `/estoque/${selectedLoja}/solares/`);
          const solaresSnapshot = await getDocs(solaresDocRef);

          solaresSnapshot.docs.forEach((docItem) => {
            const solarData = docItem.data();
            fetchedSolares.push({
              id: docItem.id,
              ...solarData,
          ...
          for (const loja of lojas) {
            const solaresDocRef = collection(firestore, `/estoque/${loja}/solares`);
            const solaresSnapshot = await getDocs(solaresDocRef);
        ...
        fetchedSolares.forEach(solar => {
          if (solar.marca && !marcas.includes(solar.marca)) {
            marcas.push(solar.marca);
          }
          if (solar.material && !materiais.includes(solar.material)) {
            materiais.push(solar.material);
          }
        });
        ...
        const sortedSolares = sortSolares(fetchedSolares, sortField, sortDirection);
        setSolares(sortedSolares);
        setFilteredSolares(sortedSolares);
        setTotalSolares(sortedSolares.length); // Atualizar o contador total de óculos solares
        setLoading(false);
      } catch (err) {
        console.error('Erro ao carregar os óculos solares:', err);
        setError(`Erro ao carregar os dados dos óculos solares: ${err.message}`);
        setLoading(false);
      }
    };
  ...
  // Função para filtrar óculos solares com base na busca e loja
  useEffect(() => {
    const filterBySearchAndLojaAndFilters = () => {
      let filtered = solares;

      // Filtro por loja
      if (selectedLoja !== 'Ambas') {
        filtered = filtered.filter((solar) => solar.loja === selectedLoja);
      }
      ...
      if (searchQuery !== '') {
        filtered = filtered.filter(
          (solar) =>
            (solar.codigo?.toLowerCase().includes(searchQuery.toLowerCase()) || '') ||
            (solar.marca?.toLowerCase().includes(searchQuery.toLowerCase()) || '') ||
            (solar.codigoBarras?.toLowerCase().includes(searchQuery.toLowerCase()) || '') ||
            (solar.codigoFabricante?.toLowerCase().includes(searchQuery.toLowerCase()) || '')
        );
      }

      // Filtro por marca
      if (marcaFilter !== 'Todas') {
        filtered = filtered.filter(solar =>
          solar.marca === marcaFilter
        );
      }
      ...
      if (materialFilter !== 'Todos') {
        filtered = filtered.filter(solar =>
          solar.material === materialFilter
        );
      }

      // Aplicar ordenação
      filtered = sortSolares(filtered, sortField, sortDirection);
      setFilteredSolares(filtered);
    };

    filterBySearchAndLojaAndFilters();
  }, [searchQuery, selectedLoja, solares, sortField, sortDirection, marcaFilter, generoFilter, materialFilter]);

  // Função para ordenar óculos solares
  const sortSolares = (solaresToSort, field, direction) => {
    return [...solaresToSort].sort((a, b) => {
      let aValue = a[field];
      let bValue = b[field];
  ...
  const handleSort = (field) => {
    ...
    // Reordenar os óculos solares filtrados
    const sorted = sortSolares(filteredSolares, field, direction);
    setFilteredSolares(sorted);
  };
  ...
  // Função para abrir o modal e definir o óculos solar selecionado
  ...
  const generatePDF = () => {
    if (!selectedSolar) return;

    const doc = new jsPDF();
    doc.text(`Detalhes do Óculos Solar`, 10, 10);
    doc.text(`Código: ${selectedSolar.codigo || 'N/A'}`, 10, 20);
    doc.text(`SKU: ${selectedSolar.sku || 'N/A'}`, 10, 30);
    doc.text(`Marca: ${selectedSolar.marca || 'N/A'}`, 10, 40);
    doc.text(`Valor: R$ ${parseFloat(selectedSolar.valor || 0).toFixed(2)}`, 10, 50);
    doc.text(`Custo: R$ ${parseFloat(selectedSolar.custo || 0).toFixed(2)}`, 10, 60);
    doc.text(`Quantidade: ${selectedSolar.quantidade || 0}`, 10, 70);
    doc.text(`Data de Cadastro: ${formatDate(selectedSolar.data)}`, 10, 80);
    doc.text(`Loja: ${selectedSolar.loja || 'N/A'}`, 10, 90);
    doc.text(`Material: ${selectedSolar.material || 'N/A'}`, 10, 100);
    doc.text(`Gênero: ${selectedSolar.genero || 'N/A'}`, 10, 110);
    doc.text(`Cor: ${selectedSolar.cor || 'N/A'}`, 10, 120);
    doc.text(`Formato: ${selectedSolar.formato || 'N/A'}`, 10, 130);

    doc.save(`Solar_${selectedSolar.codigo || selectedSolar.id}.pdf`);
  };
  ...
  // Função para marcar ou desmarcar óculos solar para exclusão
  const toggleDeletion = (e, solarId) => {
    e.stopPropagation(); // Evitar abrir o modal

    setSelectedForDeletion(prev => {
      if (prev.includes(solarId)) {
        return prev.filter(id => id !== solarId);
      } else {
        return [...prev, solarId];
      }
    });
  ...
  const handleEdit = () => {
    if (selectedForDeletion.length !== 1) {
      alert('Selecione apenas um óculos solar para editar.');
      return;
    }

    // Encontrar o óculos solar selecionado
    const solarToEdit = solares.find(solar => solar.id === selectedForDeletion[0]);

    if (solarToEdit) {
      // Navegar para a página de formulário passando os dados do óculos solar
      router.push(`/products_and_services/solar?cloneId=${solarToEdit.codigo}&loja=${solarToEdit.loja}`);
    }
  };

  // Função para excluir os óculos solares selecionados
  const handleDeleteSelected = async () => {
    if (selectedForDeletion.length === 0) {
      alert('Selecione pelo menos um óculos solar para excluir.');
      return;
    }

    if (confirm(`Deseja realmente excluir ${selectedForDeletion.length} óculos solares selecionados?`)) {
      try {
        for (const solarId of selectedForDeletion) {
          // Encontrar a loja do óculos solar
          const solar = solares.find(s => s.id === solarId);
          if (solar && solar.loja) {
            // Excluir o óculos solar
            const solarRef = doc(firestore, `/estoque/${solar.loja}/solares/`, solarId);
            await deleteDoc(solarRef);
          }
        }

        // Atualizar as listas de óculos solares
        const updatedSolares = solares.filter(solar => !selectedForDeletion.includes(solar.id));
        setSolares(updatedSolares);
        setFilteredSolares(updatedSolares.filter(s =>
          (selectedLoja === 'Ambas' || s.loja === selectedLoja) &&
          (!searchQuery ||
            s.codigo?.toLowerCase().includes(searchQuery.toLowerCase()) ||
            s.marca?.toLowerCase().includes(searchQuery.toLowerCase()))
        ));
        setTotalSolares(updatedSolares.length);
        setSelectedForDeletion([]);

        alert('Óculos solares excluídos com sucesso!');
      } catch (error) {
        console.error('Erro ao excluir óculos solares:', error);
        alert('Erro ao excluir os óculos solares selecionados.');
      }
    }
  };

  // Calcular óculos solares para a página atual
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentSolares = filteredSolares.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(filteredSolares.length / itemsPerPage);
  ...
  const generateQRCodeData = (solar) => {
  ...
  const downloadQRCode = (solar) => {
    const canvas = document.createElement('canvas');
    const qrCode = new QRCodeSVG({
      value: generateQRCodeData(solar),
      size: 200,
      level: 'H',
      includeMargin: true
    });
    ...
    img.onload = () => {
      ...
      link.download = `QRCode_${solar.codigo}.png`;
    ...
    <Layout>
      <div className="min-h-screen p-0 md:p-2 mb-20">
        <div className="w-full max-w-5xl mx-auto rounded-lg">
          <div className="mb-4">
            <h2 className="text-3xl font-bold text-[#81059e] mb-8 mt-8">ÓCULOS SOLARES REGISTRADOS</h2>
          </div>

          {/* Dashboard compacto com estatísticas essenciais */}
          <div className="mb-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
              {/* Card - Total de Óculos Solares */}
              <div className="border-2 rounded-lg p-4 border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between mb-3">
                  ...
                  />
                  <span className="text-lg font-medium text-gray-700 flex-grow ml-3">Total de Óculos Solares</span>
                </div>
                <p className="text-2xl font-semibold text-center mt-2">{totalSolares}</p>
              </div>
              ...
              <div className="border-2 rounded-lg p-4 border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-2xl font-semibold text-center mt-2">
                  R$ {solares.reduce((total, solar) =>
                    total + (parseFloat(solar.valor || 0) * parseInt(solar.quantidade || 0)), 0).toFixed(2)}
                </p>
              </div>
              ...
              <div className="border-2 rounded-lg p-4 border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-2xl font-semibold text-center mt-2">
                  {solares.filter(solar => parseInt(solar.quantidade || 0) <= 3).length}
                </p>
          ...
          <div className="flex flex-wrap gap-2 items-center mb-4">
           ...
           <div className="flex gap-2">
             ...
             {/* Botão Editar - aparece apenas quando há óculos solares selecionados */}
             <button
               onClick={() => handleEdit()}
         ...
         {loading ? (
           ...
           <div className="w-full overflow-x-auto">
             {filteredSolares.length === 0 ? (
               <p>Nenhum óculos solar encontrado.</p>
             ) : (
               <>
                       ...
                       {currentSolares.map((solar) => (
                         ...
                         >
                           <td className="border px-2 py-2 text-center">
                             <input
                               type="checkbox"
                               checked={selectedForDeletion.includes(solar.id)}
                               onChange={(e) => toggleDeletion(e, solar.id)}
                           ...
                           <td className="border px-3 py-2 whitespace-nowrap" onClick={() => openModal(solar)}>
                             {solar.codigo || 'N/A'}
                           </td>
                           <td className="border px-4 py-2 max-w-[300px] truncate" onClick={() => openModal(solar)}>
                             {solar.marca || 'N/A'}
                           </td>
                           <td className="border px-3 py-2" onClick={() => openModal(solar)}>
                             {solar.genero || 'N/A'}
                           </td>
                           <td className="border px-3 py-2" onClick={() => openModal(solar)}>
                             {solar.cor || 'N/A'}
                           ...
                           <td className="border px-3 py-2 whitespace-nowrap" onClick={() => openModal(solar)}>
                             R$ {parseFloat(solar.valor || 0).toFixed(2)}
                           </td>
                           <td className="border px-3 py-2 whitespace-nowrap" onClick={() => openModal(solar)}>
                             R$ {parseFloat(solar.custo || 0).toFixed(2)}
                           </td>
                           <td className={`border px-3 py-2 whitespace-nowrap text-center ${getEstoqueColor(solar.quantidade)}`} onClick={() => openModal(solar)}>
                             {solar.quantidade || '0'}
                           </td>
                           <td className="border px-3 py-2 whitespace-nowrap" onClick={() => openModal(solar)}>
                             {renderLojaName(solar.loja)}
                           </td>
                         </tr>
                       ))}
                     </tbody>
                 ...
                 <div className="flex items-center justify-between mt-4">
                   <div className="text-sm text-gray-700">
                     ...
                     <span className="font-medium">
                       {Math.min(indexOfLastItem, filteredSolares.length)}
                     </span>{' '}
                     de <span className="font-medium">{filteredSolares.length}</span> registros
                   </div>
         ...
         {/* Modal de Detalhamento de Óculos Solar */}
         {isModalOpen && selectedSolar && (
           <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
             <div className="bg-white rounded-lg shadow-lg w-full max-w-md flex flex-col h-3/5 overflow-hidden">
               <div className="bg-[#81059e] text-white p-4 flex justify-between items-center">
                 <h3 className="text-xl font-bold">Detalhes do Óculos Solar</h3>
               ...
               <div className="space-y-3 p-4 overflow-y-auto flex-grow">
                 {selectedSolar.imagem && (
                   <div className="flex justify-center mb-4">
                     <img
                       ...
                       alt={`Imagem de ${selectedSolar.marca}`}
                       className="max-h-32 object-contain rounded"
                       onError={(e) => {
                         e.target.onerror = null;
                         e.target.src = '/api/placeholder/200/200'; // Fallback para imagem placeholder
                         console.error('Erro ao carregar imagem:', selectedSolar.imagem);
                       }}
                     />
                   </div>
                 )}
                 ...
                 <div className="flex flex-col items-center mb-4">
                   <div className="bg-white p-4 rounded-lg shadow-md">
                     <QRCodeSVG
                       value={generateQRCodeData(selectedSolar)}
                       size={150}
                       level="H"
                       includeMargin={true}
                     />
                   ...
                   <button
                     onClick={() => downloadQRCode(selectedSolar)}
                 ...
                 <p><strong>
                   Código:</strong> {selectedSolar.codigo || 'N/A'}
                 </p>
                 <p>
                   <strong>SKU:</strong> {selectedSolar.sku || 'N/A'}
                 </p>
                 <p>
                   <strong>Marca:</strong> {selectedSolar.marca || 'N/A'}
                 </p>
                 <p>
                   <strong>Gênero:</strong> {selectedSolar.genero || 'N/A'}
                 </p>
                 <p>
                   <strong>Material:</strong> {selectedSolar.material || 'N/A'}
                 </p>
                 <p>
                   <strong>Cor:</strong> {selectedSolar.cor || 'N/A'}
                 </p>
                 <p>
                   <strong>Formato:</strong> {selectedSolar.formato || 'N/A'}
                 </p>
                 <p>
                   <strong>Valor:</strong> R$ {parseFloat(selectedSolar.valor || 0).toFixed(2)}
                 </p>
                 <p>
                   <strong>Custo:</strong> R$ {parseFloat(selectedSolar.custo || 0).toFixed(2)}
                 </p>
                 <p>
                   <strong>Margem:</strong> {selectedSolar.percentual_lucro || '0'}%
                 </p>
                 <p>
                   <strong>Quantidade:</strong> {selectedSolar.quantidade || '0'}
                 </p>
                 <p>
                   <strong>Data de Cadastro:</strong> {formatDate(selectedSolar.data)}
                 </p>
                 <p>
                   <strong>Loja:</strong> {renderLojaName(selectedSolar.loja)}
                 </p>
                 ...
                 <div className="mt-4 pt-2 border-t border-gray-200">
                   ...
                   <div className="grid grid-cols-3 gap-2 mt-2">
                     <div>
                       ...
                       <p className="font-medium">{selectedSolar.lente || 'N/A'}</p>
                     </div>
                     <div>
                       <p className="text-sm text-gray-600">Ponte</p>
                       <p className="font-medium">{selectedSolar.ponte || 'N/A'}</p>
                     </div>
                     <div>
                       <p className="text-sm text-gray-600">Haste</p>
                       <p className="font-medium">{selectedSolar.haste || 'N/A'}</p>
                     </div>
                   </div>
                 </div>
                 ...
                 <div className="mt-4 pt-2 border-t border-gray-200">
                   ...
                   <p className="mt-2">
                     <strong>NCM:</strong> {selectedSolar.NCM || 'N/A'}
                   </p>
                   <p>
                     <strong>Código de Barras:</strong> {selectedSolar.codigoBarras || 'N/A'}
                   </p>
                 </div>
                 ...
                 <div className="flex justify-around mt-4">
                   <Link href={`/products_and_services/solar?cloneId=${selectedSolar.codigo}&loja=${selectedSolar.loja}`}>
                   ...
                   <button
                     onClick={() => {
                       if (confirm('Tem certeza que deseja excluir este óculos solar?')) {
                         handleDeleteSelected([selectedSolar.id]);
                         closeModal();
                       }
                     }}
