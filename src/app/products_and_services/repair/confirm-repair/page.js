...
export function ConfirmAssembly() {
  ...
  const handleEdit = () => {
    ...
    router.push(`/products_and_services/repair?formData=${queryString}`);
  };
  ...
  const handleConfirm = async () => {
    try {
      ...
      // Primeiro, cria ou obtém uma referência para o documento da loja
      const lojaDocRef = doc(collection(firestore, 'repairs'), formDataFromQuery.loja); // Refere-se ao documento da loja
      // Agora, adiciona uma subcoleção "dados" para armazenar os malotes
      const docRef = doc(collection(lojaDocRef, 'dados'), 'reparo_' + new Date().getTime()); // Adiciona um malote com um ID único
      ...
      setTimeout(() => {
        ...
        router.push('/products_and_services/repair/list-repairs');
      }, 2000);
    } catch (error) {
      console.error('Erro ao registrar a montagem: ', error);
    }
  };
