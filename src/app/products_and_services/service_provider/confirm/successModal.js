...
const SuccessModalProvider = ({ isOpen, onClose, message, autoClose = true, autoCloseTime = 3000 }) => {
  useEffect(() => {
    if (isOpen && autoClose) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-md w-full mx-4 shadow-2xl">
        ...
        <div className="p-6 text-center">
          <div className="mb-6">
            ...
            <p className="text-gray-500 text-sm">
              O prestador foi adicionado ao sistema e pode ser visualizado na lista de prestadores registrados.
            </p>
          </div>
        ...
        <div className="flex justify-center p-6 border-t border-gray-200">
          <button
            onClick={onClose}
            className="px-4 py-3 bg-[#81059e] text-white rounded-sm hover:bg-[#6d0485] font-medium transition-colors"
          >
            Fechar
          </button>
        </div>
