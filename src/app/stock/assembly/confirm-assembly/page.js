...
export function ConfirmAssembly() {
  ...
  const handleEdit = () => {
    ...
    router.push(`/stock/assembly?formData=${queryString}`);
  };
  ...
  const handleConfirm = async () => {
    try {
      ...
      const lojaDocRef = doc(collection(firestore, 'montagens'), formDataFromQuery.loja); // Refere-se ao documento da loja
      // Agora, adiciona uma subcoleção "dados" para armazenar os malotes
      const docRef = doc(collection(lojaDocRef, 'dados'), 'malote_' + new Date().getTime()); // Adiciona um malote com um ID único
      ...
      setTimeout(() => {
        ...
        router.push('/stock/assembly/list-assemblies');
      }, 2000);
    } catch (error) {
